    ...
    def train(self, num_episodes, env):
        ...
        for episode in range(num_episodes):
            ...
            while not done and training_running:
                next_state, target_action, done = env.step()
                if done:
                    break
                self.replay_buffer.append([state, target_action])
                state = next_state
...
class OsuManiaEnv(gym.Env):
    def __init__(self):
        ...
        self.keys = ['d', 'f', 'j', 'k']
        self.pink = cv2.imread('Osu_ai/pink.png')
        self.white = cv2.imread('Osu_ai/white.png')
    ...
    def step(self):
        ...
        with mss.mss() as sct:
            ...
            img = np.array(sct.grab(rect))
            img = cv2.cvtColor(img, cv2.COLOR_RGBA2RGB)
        state = self.preprocess_screen(img[824:924, 300:580])
        ...
        return state, target_action, done
    ...
    def extract_action(self, img1, img2, img3, img4):
        action=np.zeros(4)
        res = cv2.matchTemplate(img1, self.white, cv2.TM_CCOEFF_NORMED)
        loc = np.where(res >= 0.1)
        if len(loc[0]) > 0:
            action[0]=1
        res = cv2.matchTemplate(img2, self.pink, cv2.TM_CCOEFF_NORMED) 
        loc = np.where(res >= 0.8)
        if len(loc[0]) > 0:
            action[1]=1
        res = cv2.matchTemplate(img3, self.pink, cv2.TM_CCOEFF_NORMED) 
        loc = np.where(res >= 0.8)
        if len(loc[0]) > 0:
            action[2]=1
        res = cv2.matchTemplate(img4, self.white, cv2.TM_CCOEFF_NORMED) 
